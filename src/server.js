...
async function loadIpList() {
  ipList = await new Promise((resolve, reject) => {
...
loadIpList();
setInterval(() => pingIp(ipList), 30 * 1000);
pingIp(ipList);

// 路由
app.get('/', (req, res) => {
  db.all(`SELECT * FROM ping_records ORDER BY timestamp DESC LIMIT 50`, (err, records) => {
    if (err) return res.send('获取状态记录失败');
    let html = `
      <h1>IP 状态监控</h1>
      <table border="1">
        <tr><th>IP</th><th>编号</th><th>时间</th><th>状态</th><th>延迟 (ms)</th></tr>`;
